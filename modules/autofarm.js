...
function onSave() {
  ...
  const config = {
    ...
    packSaveWorldInput,
    saveSeed,
    saveSeedWOrldInput: saveSeedWorldInput,
    magplantMode
  }
...
function onRender() {
  gui.child("ac_1", new vector2(220, 245), () => {
    ...
    if (gui.checkbox("Magplant Mode", magplantMode)) magplantMode = !magplantMode
...
function loadConfig(configSave) {
  if (configSave) {
    ...
    magplantMode = config.magplantMode
  }
}
...
function doAutofarm() {
  ...
  for (let i = 0; i < tiles.length; i++) {
    ...
    if (magplantMode) {
      bot.place(pos, 5640, enableVisuals)
    }
    else {
