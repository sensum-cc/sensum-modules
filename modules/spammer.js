...
function onRender() {
  gui.child("sc1", new vector2(220, 230), () => {
    ...
    if (gui.checkbox("Auto Exit", autoExit)) autoExit = !autoExit
    gui.text("World Name")
...
function main(configSave) {
  ...
  if (positionInput != "0,0" && positionInput != "") {
    ...
    pos.x -= 1
    pos.y -= 1
    if (pos != new vector2Int(0, 0)) if (bot.findPath(pos) == false) return
  }
  ...
  while (isEnabled()) {
    ...
    if (ix === 0 && spamTextInput === "") {
      ix++
      if (ix > 3) ix = 0
      continue
    }
    if (ix === 1 && spamTextInput2 === "") {
      ix++
      if (ix > 3) ix = 0
      continue
    }
    if (ix === 2 && spamTextInput3 === "") {
      ix++
      if (ix > 3) ix = 0
      continue
    }
    if (ix === 3 && spamTextInput4 === "") {
      ix++
      if (ix > 3) ix = 0
      continue
    }
